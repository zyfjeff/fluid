...
func TestDestroyWorker(t *testing.T) {
	...
	runtimeInfoSpark, err := base.BuildRuntimeInfo("spark", "fluid", "cachefs", datav1alpha1.TieredStore{})
	if err != nil {
	...
	runtimeInfoHadoop, err := base.BuildRuntimeInfo("hadoop", "fluid", "cachefs", datav1alpha1.TieredStore{})
	if err != nil {
		t.Errorf("fail to create the runtimeInfo with error %v", err)
	}
	...
	var nodeInputs = []*corev1.Node{
		{
			ObjectMeta: metav1.ObjectMeta{
				...
				Labels: map[string]string{
					...
					"fluid.io/s-cachefs-fluid-spark":     "true",
					"fluid.io/s-fluid-spark":             "true",
					"fluid.io/s-h-cachefs-d-fluid-spark": "5B",
					"fluid.io/s-h-cachefs-m-fluid-spark": "1B",
					"fluid.io/s-h-cachefs-t-fluid-spark": "6B",
		...
		},
		{
			ObjectMeta: metav1.ObjectMeta{
				...
				Labels: map[string]string{
					...
					"fluid.io/s-cachefs-fluid-hadoop":     "true",
					"fluid.io/s-fluid-hadoop":             "true",
					"fluid.io/s-h-cachefs-d-fluid-hadoop": "5B",
					"fluid.io/s-h-cachefs-m-fluid-hadoop": "1B",
					"fluid.io/s-h-cachefs-t-fluid-hadoop": "6B",
					"fluid.io/s-cachefs-fluid-hbase":      "true",
					"fluid.io/s-fluid-hbase":              "true",
					"fluid.io/s-h-cachefs-d-fluid-hbase":  "5B",
					"fluid.io/s-h-cachefs-m-fluid-hbase":  "1B",
					"fluid.io/s-h-cachefs-t-fluid-hbase":  "6B",
				},
			},
		},
		{
	...
	for _, test := range testCase {
		engine := &CacheFSEngine{Log: fake.NullLogger(), runtimeInfo: test.runtimeInfo}
...
func TestCacheFSEngine_destroyMaster(t *testing.T) {
...
func TestCacheFSEngine_cleanupCache(t *testing.T) {
	testRuntime := &datav1alpha1.CacheFSRuntime{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "test",
			Namespace: "fluid",
		},
	...
	testRuntimeWithTiredStore := &datav1alpha1.CacheFSRuntime{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "test2",
			Namespace: "fluid",
		},
	...
	testConfigMap := &corev1.ConfigMap{
		ObjectMeta: metav1.ObjectMeta{Name: "test-cachefs-values", Namespace: "fluid"},
		Data:       map[string]string{"data": "{\"source\": \"test\"}"},
	}
	...
	Convey("Test CleanupCache ", t, func() {
		Convey("cleanup success", func() {
			...
			defer patch1.Reset()
			patch2 := ApplyMethod(reflect.TypeOf(&operations.CacheFSFileUtils{}), "DeleteCacheDirs",
				func(_ *operations.CacheFSFileUtils, cacheDirs []string) error {
					return nil
				})
			defer patch2.Reset()
			patch3 := ApplyMethod(reflect.TypeOf(&operations.CacheFSFileUtils{}), "GetStatus",
				func(_ *operations.CacheFSFileUtils, source string) (status string, err error) {
		...
		Convey("test1", func() {
			...
			defer patch1.Reset()
			patch2 := ApplyMethod(reflect.TypeOf(&operations.CacheFSFileUtils{}), "DeleteCacheDir",
				func(_ *operations.CacheFSFileUtils, cacheDir string) error {
...
func TestCacheFSEngine_getUUID(t *testing.T) {
	testConfigMap := &corev1.ConfigMap{
		ObjectMeta: metav1.ObjectMeta{Name: "test-cachefs-values", Namespace: "fluid"},
		Data:       map[string]string{"data": "{\"source\": \"test\"}"},
	}
	...
	ExecCommon := func(a *operations.CacheFSFileUtils, source string) (status string, err error) {
		return CommonStatus, nil
	}
	ExecErr := func(a *operations.CacheFSFileUtils, source string) (status string, err error) {
		return "", errors.New("fail to run the command")
	}
	wrappedUnhookExec := func() {
		err := gohook.UnHook((*operations.CacheFSFileUtils).GetStatus)
		if err != nil {
			t.Fatal(err.Error())
		}
	}
	...
	err := gohook.Hook((*operations.CacheFSFileUtils).GetStatus, ExecErr, nil)
	if err != nil {
		t.Fatal(err.Error())
	}
	_, err = e.getUUID(pod, common.CacheFSWorkerContainer)
	if err == nil {
		t.Error("getUUID failure, want err, got nil")
	}
	wrappedUnhookExec()

	err = gohook.Hook((*operations.CacheFSFileUtils).GetStatus, ExecCommon, nil)
	if err != nil {
		t.Fatal(err.Error())
	}
	got, err := e.getUUID(pod, common.CacheFSWorkerContainer)
	if err != nil {
...
func TestCacheFSEngine_cleanAll(t *testing.T) {
...
func TestCacheFSEngine_getCacheDirs(t *testing.T) {
	type args struct {
		runtime *datav1alpha1.CacheFSRuntime
	}
	tests := []struct {
