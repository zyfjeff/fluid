...
func TestCacheFSEngine_genValue(t *testing.T) {
	...
	testObjs := []runtime.Object{}
	testObjs = append(testObjs, (*cachefsSecret1).DeepCopy())
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for k, v := range opt {
				if v1, ok := tt.wantOptions[k]; !ok {
					t.Errorf("CacheFSEngine.genUFSMountOptions() should has key: %v", k)
				} else {
					if v1 != v {
						t.Errorf("CacheFSEngine.genUFSMountOptions()  key: %v value: %v, get value: %v", k, v1, v)
					} else {
...
func TestCacheFSEngine_genMount(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantErr:           false,
			wantWorkerCommand: "/bin/mount.cachefs redis://127.0.0.1:6379 /test-worker -o metrics=0.0.0.0:9567",
			wantFuseCommand:   "/bin/mount.cachefs redis://127.0.0.1:6379 /test -o metrics=0.0.0.0:9567",
		...
		},
		{
			...
			args: args{
				...
				runtime: &datav1alpha1.CacheFSRuntime{Spec: datav1alpha1.CacheFSRuntimeSpec{Worker: datav1alpha1.CacheFSCompTemplateSpec{
					Options: map[string]string{"metrics": "127.0.0.1:9567"},
				}}},
			},
			wantErr:           false,
			wantWorkerCommand: "/bin/mount.cachefs redis://127.0.0.1:6379 /test-worker -o verbose,metrics=127.0.0.1:9567",
			wantFuseCommand:   "/bin/mount.cachefs redis://127.0.0.1:6379 /test -o verbose,metrics=0.0.0.0:9567",
			wantFuseStatCmd:   "stat -c %i /test",
			wantWorkerStatCmd: "stat -c %i /test-worker",
		},
	}
